...
variable "clusters" {
  ...
  type = map(object({
    ...
    node_classes             : map(object({
      ...
      pve_nodes              : optional(list(string),["pve"]) # Optional. Nodes that this class is allowed to run on. They will be cycled through and will repeat if count > length(pve_nodes).
    ...
    alpha = {
      ...
      networking = {
        ipv4 = {
          ...
          gateway          = "192.168.0.1"
          management_cidrs = "192.168.0.110/32"
          lb_cidrs         = "192.168.0.200/29"
          dns1             = "192.168.0.101"
          dns2             = "1.1.1.1"
        }
